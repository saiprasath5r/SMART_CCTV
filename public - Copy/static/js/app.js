...
class SurveillanceDashboardApp {
  ...
  _initWebSocket() {
    // When deployed to Firebase Hosting, WebSockets cannot be proxied through
    // Firebase rewrites. Use BACKEND_URL (Cloud Run URL) if available, otherwise
    // fall back to current host (local development).
    let wsUrl;
    if (window.BACKEND_URL && window.BACKEND_URL !== '%%BACKEND_URL%%' && window.BACKEND_URL !== '') {
      // Cloud Run URL → convert https:// to wss://
      const backendBase = window.BACKEND_URL.replace(/^https?:\/\//, '');
      const wsProtocol = window.BACKEND_URL.startsWith('https') ? 'wss:' : 'ws:';
      wsUrl = `${wsProtocol}//${backendBase}/ws/stream`;
    } else {
